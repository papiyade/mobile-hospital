...
class _AppointmentsScreenState extends State<AppointmentsScreen> {
  ...
  Future<void> _load({bool isRefresh = false}) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => error = "Erreur de chargement");
      }
    } finally {
      if (mounted) {
        setState(() {
...
InkWell(
  ...
  child: Container(
    ...
    child: Row(
      children: [
        const Icon(Icons.access_time, color: MediQueueTheme.primary),
        const SizedBox(width: 10),
        Text(
          selectedTime == null
              ? "Choisir une heure"
